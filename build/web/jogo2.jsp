...
<html>
    ...
    <body class="fundo2" <%
        ...
        <div class="caixa100">
            <% if((contJog2 == 12) || (pontOponente < 0)){out.println("<center><img class='vitoria' src='img/confetes.gif' /></center><input type='button' class='fim' value='Fim!' onclick='fim()'>");}%>
            <% if((contJog1 == 12) || (pontuacao < 0)){out.println("<img class='gameOver' src='img/gameover.gif' /><input type='button' class='fim' value='Fim!' onclick='fim()'>");}%>
            ...
            <div class="tabuleiro">
                <form action="Servlet?id=<%=id%>&nome=<%=nome%>&dificuldade=<%=dificuldade%>" method="POST">
                    <table>
                        <% if(jogadas % 2 == 0 && contJog1 < 12 && contJog2 < 12 && pontuacao >= 0 && pontOponente >= 0){out.print("<img class='aguarde' src='img/aguarde.gif' />");}%>
                        <%  int cont = 0;
                            for (int i = 0; i < 8; i++) {
                                out.print("<tr>");
                                for (int j = 0; j < 8; j++) {
                                    out.print("<td><input type='submit' class='botao' style='background: url(\"img/"+ pasta +"/"+ bt[cont] +".gif\"' name='botao2" + cont + "' value='" + (cont+1) + "' " + btoff[cont] + "></td>");
                                    cont++;
                                }
                                out.print("</tr>");
                            }
